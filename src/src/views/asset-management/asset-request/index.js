...
import RequestDAta from './asset-req.json'
...
const DropRequestComponent = ({ data }) => {
  ...
  const status = {
    0: { title: 'Confirmer', color: 'light-info' },
    1: { title: 'Reporté', color: 'light-primary' },
    2: { title: 'Ordre généré', color: 'light-dark' },
    3: { title: 'Terminé', color: 'light-success' },
    4: { title: 'Annulé', color: 'light-danger' }
  }
  ...
  const columns = [
    {
      ...
      cell: row => {
        return (
          <Link to={`/asset/request/edit/${row.XDROPID_0}`}>
            {row.XDROPID_0}
          </Link>
        )
      }

...
const initialState = {
 ...
 const [isSubmitted, setSubmitted] = useState(false)
  ...
  const summaryOptions  = () => {
      console.log("AT present current time is", currTime)
      const d = new Date()
      addresses && addresses.length > 0 ? d.setDate(d.getDate() + addresses[selectAddressIndex].LEADTIME) : Date()
     console.log("lead time =", addresses && addresses.length > 0 && new Date() + 5)
console.log("lead time 2 =", d)
      setDropRequest({
          ...dropRequest,
       ...
       setDeliveryDate(moment(d).format('L'))
    }
...
const handleAddressChange = (e) => {
  setSelectAddress(e.target.value)
   let addressID = '', dlvymode = '', carrieropt = '', ddate = new Date()
     console.log("before change date is", dropRequest.date)
  for (let i = 0; i < addresses.length; i++) {
      if (addresses[i].BPADES_0 === e.target.value) {
           ...
           addressID = addresses[i].BPAADD_0 !== null ? addresses[i].BPAADD_0 : ''
            ...
            // ddate = dropRequest.date.setDate(dropRequest.date + addresses[i].LEADTIME)
            ddate = moment(ddate).add(addresses[i].LEADTIME, 'days').format('L')
      }

  }
        console.log("after change date is", ddate)
 setDropRequest({ ...dropRequest, ['address']: e ? addressID : '', ['carrier']: carrieropt, ['delivery_mode']: dlvymode, ['date']: ddate, ['dateflg'] : 1  })
            ...
            .then(data => {
              toast.success(
                ...
                { transition: Slide, hideProgressBar: true, autoClose: 2000 }
                 ...
                 history.push('/customer/order-requests/shop')
            })
        ...
        <CardBody>
          ...
          <Col md='6' sm='12'>
                        <FormGroup className='mb-2'>
                          ...
                          <Input type='select' name='add-type' id='add-type' onChange={handleAddressChange} value={selectAddress}>
            ...
            <Col md='6' sm='12'>
                          ...
                          <Col md='6' sm='12'>
                          <FormGroup className='mb-2'>
                                                      <Label for='checkout-landmark'>Lead Days</Label>
                                                      <Input
                                                        name='checkout-landmark'
                                                        id='checkout-landmark'
                                                      placeholder= {addresses && addresses.length > 0 ? addresses[selectAddressIndex].LEADTIME : ''}
                                                        innerRef={register({ required: true })}
                                                        className={classnames({ 'is-invalid': errors['checkout-landmark'] })}
                                                      />
                                                    </FormGroup>
            ...
            <Col sm='12' md='6'>
            <FormGroup>
                                                        <Label for='date'>Requested Delivery Date <span className='text-danger'>*</span></Label>
                                                            <Flatpickr style={{ backgroundColor : dropRequest.dateflg && dropRequest.dateflg === 1 ? "green" : "red"}}
                                                              value={dropRequest.date}
                                                              onChange={(date) => setDropRequest({...dropRequest, ['date']: date[0], ['dateflg'] :  new Date(date[0]).getTime() >= new Date(deliveryDate).getTime() ? 1 : 2 })}
                                                              options={{
                                                                dateFormat: 'm/d/Y',
                                                                minDate: 'today'
                                                              }}

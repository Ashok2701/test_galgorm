...
const CreateDocumentComponent = (props) => {
  ...
  const businessPartnerOptions = [{value: 'All', label: 'ALL (Tous les partenaires commerciaux)'}]
  ...
  const onSubmit = async (data) => {
    ...
    if (isObjEmpty(errors) && document.description) {
      ...
      if (document.id) {
        return mySwal.fire({
          title: 'Êtes-vous sûr ?',
          text: "Vous n'aurez pas à mettre à jour ce document !",
          icon: 'warning',
  ...
  return (
    <div className="w-100">
      <CardTitle tag='h4'>{document.id ? 'DOCUMENT DE MISE À JOUR' : 'CRÉER UN DOCUMENT'}</CardTitle>
      <Row>
        <Col sm='12'>
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Card>
              <CardBody className="pt-1">
                ...
                <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='x3user'>Partenaire commercial <span className='text-danger'>*</span></Label>
                      <Select
                        id='x3user'
                        theme={selectThemeColors}
                        classNamePrefix='select'
                        name='x3user'
                        placeholder='Sélectionner le partenaire commercial'
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='type'>Type de document <span className='text-danger'>*</span></Label>
                      <Input
                        name='type'
                        id='type'
                        value={document.type}
            ...
            <Button type='submit' onClick={() => setSubmitted(true)} className='mr-1' color='primary'>
              {document.id ? 'Mise à jour' : 'Créer'}
            </Button>
            {/* <Button type='reset' color='secondary' className='mr-1' outline tag={Link} to='/users'>Cancel</Button> */}
            <Button type='button' color='danger' outline tag={Link} to='/admin/document'>Retour</Button>
          </Form>
        </Col>
      </Row>
    </div>

...
const CreateUserComponent = (props) => {
  ...
  const onSubmit = () => {
    ...
    if (isObjEmpty(errors) && user.role && user.concode && user.x3user && user.site) {
      if (user.id) {
        return mySwal.fire({
          title: 'Êtes-vous sûr ?',
          text: "Vous ne pourrez pas mettre à jour cet utilisateur !",
          icon: 'warning',
  ...
  return (
    <div className="w-100">
      <CardTitle tag='h4'>{user.id ? 'MISE À JOUR DE LUTILISATEUR' : 'CRÉER UN NOUVEL UTILISATEUR'}</CardTitle>
      <Row>
        <Col sm='12'>
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Card>
              <CardBody className="pt-1">
                <Row>
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='role'>Rôle <span className='text-danger'>*</span></Label>
                      <Select
                        id='role'
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='x3user'>{user.role === 2 ? 'Client' : 'Fournisseur'} <span className='text-danger'>*</span></Label>
                      <Select
                        id='x3user'
                        theme={selectThemeColors}
                        classNamePrefix='select'
                        name='x3user'
                        placeholder='Sélectionner lutilisateur'
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='contact'>Code de contact<span className='text-danger'>*</span></Label>
                      <Select
                        id='contact'
                        theme={selectThemeColors}
                        classNamePrefix='select'
                        name='contact'
                        placeholder='Sélectionner le code de contact'
                  ...
                  <Col md='4' sm='12' className="mt-2">
                    <FormGroup className="d-flex align-items-center">
                      <Label for='status' className="mb-0 mr-1">Statut</Label>
                      <CustomInput
            ...
            <Card>
              <CardBody className="pt-1">
                ...
                { user.role !== 4 ? (
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='xlogin'>Identifiant de connexion <span className='text-danger'></span></Label>
                      <Input
                        name='xlogin'
                        id='xlogin'
                        value={user.xlogin}
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='password'>Mot de passe <span className='text-danger pr-1'>*</span>
                        <Info size={16} id='infoIcon' />
                        <UncontrolledTooltip placement="top" target="infoIcon">
                       Au moins huit caractères, dont au moins une lettre, un chiffre et un caractère spécial.
                        </UncontrolledTooltip>
                      </Label>
                      <InputPasswordToggle
                        value={user.password}
                        ...
                        innerRef={register({ 
                          ...
                          pattern: {
                            ...
                            message: "Au moins huit caractères, dont au moins une lettre, un chiffre et un caractère spécial."
                          },
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='name'>Nom complet <span className='text-danger'>*</span></Label>
                      <Input
                        name='name'
                        id='name'
                        value={user.name}
                  ...
                  <Col md='4' sm='12'>
                    <FormGroup>
                      <Label for='email'>Courriel <span className='text-danger'>*</span></Label>
                      <Input
                        name='email'
                        id='email'
                        value={user.email}
            ...
            { user.role && user.role !== '' ? (
              <Card>
                <CardBody className="pt-1">
                  <h5 className="text-medium mb-2">Autoriser laccès</h5>
                  <Row>
                    { user.role === 2 ? (
                      <Col sm='12'>
                        <FormGroup>
            ...
            <Button type='submit' onClick={() => setSubmitted(true)} className='mr-1' color='primary'>
              {user.id ? 'Mise à jour' : 'création'}
            </Button>
            {/* <Button type='reset' color='secondary' className='mr-1' outline tag={Link} to='/users'>Cancel</Button> */}
            <Button type='button' color='danger' outline tag={Link} to='/admin/users'>Retour</Button>
          </Form>
        </Col>
      </Row>
    </div>

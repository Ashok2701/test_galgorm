  ...
  const onSubmit = () => {
    ...
    if (isObjEmpty(errors) && discussionForum.description) {
      if (discussionForum.id) {
        return mySwal.fire({
          title: 'Êtes-vous sûr ?',
          text: "Vous n'aurez pas à mettre à jour ce forum de discussion !",
          icon: 'warning',
    ...
    return mySwal.fire({
      ...
      if (result.value) {
        dispatch(deleteDiscussionForum(id))
          .then(data => {
            toast.success(
              <ToastContent header='Success' message='Suppression réussie' color='success' />,
              { transition: Slide, hideProgressBar: true, autoClose: 2000 }
            )
          })
          .catch(e => {
    ...
    <div className="w-100">
        ...
        <Col sm='12'>
          <Card>
            <CardHeader className='flex-md-row flex-column align-md-items-center border-bottom'>
              <CardTitle tag='h4'>Forums de discussion</CardTitle>
            </CardHeader>
      ...
      <Modal isOpen={basicModal} toggle={() => setBasicModal(!basicModal)}>
        <Form onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader toggle={() => setBasicModal(!basicModal)}>Ajouter un nouveau forum de discussion</ModalHeader>
          <ModalBody>
            <Row>
              {/* <Col sm='12'>
                <FormGroup>
              ...
              <Col sm='12'>
                <FormGroup>
                  <Label for='title'>Titre<span className='text-danger'>*</span></Label>
                  <Input
                    name='title'
                    id='title'
          ...
          <ModalFooter>
            <Button type='submit' color='primary' onClick={() => setSubmitted(true)}>
              {discussionForum.id ? 'Mise à jour' : 'Créer'}
            </Button>
          </ModalFooter>
        </Form>
      </Modal>

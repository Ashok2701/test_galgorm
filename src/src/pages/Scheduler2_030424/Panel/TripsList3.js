...
import { convertHrToSec,formatHHMMintoSec, convertSecToHr, convertMinToSec, formatTime, nullAndNanChecking, splitTime,formatHrMin,convertMinToHr, } from '../converterFunctions/converterFunctions';
    ...
    onValidateYes = (index) => {
     ...
     onOptimiseConfirm = (trip,index, opti, lock) => {
            ...
            if (lock) {
                    ...
                    if(opti === 'Optimized') {
                      this.setState({
                                            confirmMessage: this.props.t('ReoptimiseTrip'),
                    ...
                    this.setState({
                        confirmMessage: this.props.t('optimiseTrip'),
                        addoptimiseconfirmShow: true,
                        index: index,
                    });
                    }
 ...
 loadingSecs = (hr, min, loadingHrs) => {
     ...
     optimizeRoute = (data, SeletedI) => {
            ...
            let jsonUrl;
             ...
             jsonUrl = `/json?computeBestOrder=false&routeRepresentation=summaryOnly&computeTravelTimeFor=all&routeType=shortest&avoid=unpavedRoads&travelMode=truck&vehicleMaxSpeed=${data.techObject.maxspeed}&vehicleWeight=${data.techObject.capacities}&vehicleLength=${((data.techObject.length) / 100)}&vehicleWidth=${((data.techObject.width) / 100)}&vehicleHeight=${((data.techObject.heigth) / 100)}&vehicleCommercial=true&vehicleLoadType=otherHazmatGeneral&vehicleEngineType=combustion&key=ctZzLlfGUpaNdfHiIobOeub8NBzzGkNG`;
            ...
            data.totalObject.selectedTripData.map((tripData) => {
                if (Object.keys(tripData).length > 0) {
                    let tripserTime =  formatHHMMintoSec(tripData.serviceTime)
                    serviceTime.push(tripserTime);
       ...
       tempOptimizeRoute = (data, SeletedI) => {
           ...
           let passedData = data;
            ...
            let jsonUrl;
             apiurl = 'https://api.tomtom.com/routing/1/calculateRoute/';
             jsonUrl = `/json?computeBestOrder=false&routeRepresentation=summaryOnly&computeTravelTimeFor=all&routeType=shortest&avoid=unpavedRoads&travelMode=truck&vehicleMaxSpeed=0&vehicleWeight=0&vehicleLength=0&vehicleWidth=0&vehicleHeight=0&vehicleCommercial=true&vehicleLoadType=otherHazmatGeneral&vehicleEngineType=combustion&key=ctZzLlfGUpaNdfHiIobOeub8NBzzGkNG`;
                              ...
                              //  setHandleDateChange = departure;
                                ...
                                legs.forEach((data, index) => {
                                    ...
                                    if (Number(waitingTime[index])) {
                                        waitSec = waitSec + Number(waitingTime[index])
                                    } else {
                                        waitSec = waitSec + 0;
                                    }
                                    let serTime = formatTime(Number(serviceTime[index]));
                                    let waitTime = formatTime(Number(waitingTime[index]));
                                    ...
                                    var arrivalTime = endTimeSec -
                                        (Number(serviceTime[index])) - (Number(waitingTime[index]));
                                ...
                                let maxTotTimeSec = convertHrToSec(Number(data.techObject.maxtotaltime));
                                if (prevTripsDist >= data.techObject.maxtotaldist
                                    || prevTripsTime >= maxTotTimeSec
                                    || optimizationStatus
                                    || tripsClosed) {
                                    if (prevTripsDist >= data.techObject.maxtotaldist) {
                                        setDistErrorMessage(`The vehicle cannot perform trip more than ${data.techObject.maxtotaldist} KM, please review trip documents.`)
                                        setDistError(true);
                                    } else if (prevTripsTime >= maxTotTimeSec) {
                                        setTimeErrorMessage(`The vehicle cannot perform trip more than ${data.techObject.maxtotaltime} Hrs, please review trip documents.`)
                                        setTimeError(true);
                                    } else if (tripsClosed) {
                                   ...
                                   // setHandleDateChange = selectedDate;
                                    ...
                                    let tripData = {
                                        ...
                                        tripTotalTime: convertSecToHr(tTime + serTime + waitTime + loadingHrs),
 ...
 getPOExchangeinfo(freqtrue,poprocessed,index, code) {
    ...
    getStatus(trip , valid , lock , index , docStatus,validatedflg, lvsStatus) {
        ...
        if(trip.optistatus == "Open") {
           ...
           return <span class='badge badge-success text-uppercase' style={{ fontSize: 14  }}>{this.props.t('VALIDATED')}</span>;
   }
                          else {
               return <span class='badge badge-success text-uppercase' style={{ fontSize: 14  }}>{this.props.t('DISPATCHED')}</span>;
         }
    }
  ...
  getTripStatus(trip,valid, lock, index,validatedflg, lvsStatus)
  {
    ...
    getValidatebtn(trip,valid, lock, index, docStatus,validatedflg) {
        if (!valid && lock) {
            return (
                <span
                    onClick={() => this.CheckValiationStatus(index, trip.itemCode)}> <ListAltIcon  color="primary" style={{ fontSize: 36 , float : "left" }} />
                </span>
            );
        } else if (valid && lock ) {
                             return (
        ...
        if ((trip.optistatus === "Optimized" || trip.optistatus === "optimized")) {
            ...
            if (this.props.tripsList && this.props.tripsList.length > 0) {
                this.props.tripsList.map((allTrips) => {
                    if (allTrips.code === trip.code) {
                        if (allTrips.optistatus === "Optimized") {
                            loadHrs = trip.techObject.enddepotserv + trip.techObject.startdepots;
                ...
                if (sameTrips.length > 0) {
                    ...
                    return splitTime(trip.techObject.starttime);
                }
            } else {
                return trip.techObject.starttime
            }
        }
    }
    /*
                            ...
                            <table className="table m-0">
                                <thead>
                                    <tr className="">
                                        ...
                                        <th> {this.props.t('Route Date')}</th>
                                        <th> {this.props.t('Route Details')}</th>
                                        <th width="6%"> {this.props.t('Technician ID')}</th>
                                        <th width="6%"> {this.props.t('Tech Name')}</th>
                                        ...
                                        <th width="4%"> {this.props.t('Seq')} #</th>
                                         ...
                                         <th width="4%"> {this.props.t('Actions')}</th>
                                ...
                                <tbody>
                                    {(this.props.tripsList && this.props.tripsList || []).map((trip, i) => (
                                        ...
                                        <tr className="bg-blue" style={{ backgroundColor: this.getBgcolor(trip.code, trip.optistatus) }} key={i}>
                                            ...
                                            <td width="6%">{trip.techObject.name}</td>

    ...
    addDrop = (currentTrip, data, trip) => {
        ...
        if (currentTrip.length > 0) {
         ...
         if(trip.trailers > 0) {
            ...
            if(trip.techObject.trailer && trip.techObject.trailer != ''){
               triptriler1 = trip.techObject.trailer;
                if(trip.trailers === 2) {
      ...
      //vehicle and prod category
        if(dropCompatability === true) {
          if (trip.techObject.tclcod === '') {
            dropCompatability = true;
          }
           else {
            // need to check the vehicle and products category compatability;
            for (var i = 0; i < data.products.length; i++) {
                if (trip.techObject && trip.techObject.tclcod &&
                    trip.techObject.tclcod.includes(data.products[i].productCateg)) {
                    dropCompatability = true;
                }
                else {
                    dropCompatability = false;
...
//to  check customer compatability
    ...
    addPickup = (currentTrip, data, trip) => {
        ...
        if (trip.techObject.tclcod === '') {
            ...
            // need to check the venicle and products category compatability;
            for (var i = 0; i < data.products.length; i++) {
                if (trip && trip.techObject && trip.techObject.tclcod && trip.techObject.tclcod.includes(data.products[i].productCateg)) {
                    pickupCompatability = true;
                }
                else {
                    pickupError = "product";
...
/*
        if (trip.vol <= (dropVolume + pickupVolume) ||
            trip.techObject.maxordercnt <= (trip.pickupObject.length + trip.dropObject.length) ||
            trip.techObject.capacities <= (pickupCapacities + dropCapacities)) {
      ...
      //  } else {
            ...
            if (this.props.updatedArrSite && this.props.updatedArrSite.length > 0) {
                ...
                if (trip.depSite && trip.depSite.length > 0) {
                    ...
                    trip.depSite = trip.techObject.startdepotn
                }
            }
            trip.arrSite = trip.techObject.enddepotname
            if (this.state.siteValue.length > 0) {
                trip.techObject.enddepotname = this.state.siteValue;
                trip.arrSite = this.state.siteValue
            }
            if (this.state.siteStartValue.length > 0) {
                trip.techObject.startdepotn = this.state.siteStartValue;
                trip.depSite = this.state.siteStartValue
            }
            if (this.state.siteValueTripList.length > 0) {
                trip.techObject.enddepotname = this.state.siteValueTripList;
                trip.arrSite = this.state.siteValueTripList;
            }
            if (this.state.siteStartValueTripList.length > 0) {
                trip.techObject.startdepotn = this.state.siteStartValueTripList;
  ...
  handleCheckboxChange = (event) => {
            ...
            if (this.props.tripsPanel && this.props.tripsPanel.length > 0) {
                this.props.tripsPanel.map((tripsPanel) => {
                    ...
                    if (tripsPanel.code === trip.code && tripsPanel.optistatus === "Optimized" && tripsPanel.docdate === trip.docdate) {
                        // 
                        // 
                        loadHrs = tripsPanel.techObject.enddepotserv + tripsPanel.techObject.startdepots;
                        // 
    ...
    startDepo(trip) {
        if (trip.resetdepsite) {
            ...
            return trip.techObject.startdepotn
        }
    }
                            ...
                            <table class="table">
                                <thead>
                                    <tr >
                                        ...
                                        <th width="6%">{this.props.t('Technician')}</th>
                                                ...
                                                <td onClick={this.state.isValidPassword !== 'yes' && !trip.lock ? () => this.onDepSiteClick() : null}>
                                                    {this.state.isValidPassword === 'yes' && !trip.lock ?
                                                        <div style={{ width: '100px' }}>
                                                            <Select
                                                                value={this.dropDownVal(trip.techObject.startdepotn, options, "start")}
                                                ...
                                                <td >
                                                    <div style={{ width: '100px' }}>
                                                        {trip && trip.lock ? trip.arrSite :
                                                            <Select
                                                                value={this.dropDownVal(trip.techObject.enddepotname, options, "end")}

...
class ExternalDragDrop extends Component {
    ...
    getConsultantDesignation(value) {
        ...
        return value.resourceData.skillList;
    }
  ...
  onInfoClick = (value) => {
   ...
   vehicleDetails.Technician = data.name && data.name;
   vehicleDetails.Site = data.startdepotn && data.startdepotn;
   vehicleDetails.skills = data.skillList && data.skillList;
 vehicleDetails.startTime = data.starttime && data.starttime + '  HH:MM' ;
 vehicleDetails.OverTimeStart = data.overtimestar && formatTime(convertHrToSec(data.overtimestar)) + ' Hrs';

      this.setState({
        addInfoShow: true,
        speciality: vehicleDetails,
        clickedVeh : value.resourceData.techId,
      });
    }
    ...
    resourceHeaderTemplate(props) {
        ...
        <div className="specialist-category">
            <div className={"specialist-image " + this.getConsultantImage(props)}></div> <br /> <br />
            <div className="specialist-name">{this.getConsultantName(props)}</div> <br />
           </div>
            </div>);
    }
    treeTemplate(props) {
 ...
 console.log("T333 at drop event - schduler_data =",resourceDetails);
 //to  check customer compatability
         if (dropCompatability == true) {
              // console.log("T1111 inside few customers ");
                         ...
                         console.log("T333 at drop event - schduler_data tech skills =",resourceDetails.resourceData.skillIds);
                         console.log("T333 at drop event - schduler_data data =",data.skillId);
                 if (resourceDetails.resourceData.skillIds && !resourceDetails.resourceData.skillIds.includes(data.skillId)) {
                        dropCompatability = false;
                         // console.log("T1111 inside customer doesn't match");
                         error = 'Skill';
                        this.setState({
                                                   errorType : 'Skill',
                                                    errorMessage: `Service Response Skilll (${data.skillreq}) Doesnt match with Technician Skill(${resourceDetails.resourceData.skillList})`,
                                                    addAlertShow: true,
                                                    error: true
                                                });
                                            }
        ...
        if(dropCompatability) {
                                          ...
                                          let eventData : {[key: string]: Object} = {
                                              ...
                                              techId: resourceDetails.resourceData.techId,
                                              TechnicianObject : resourceDetails.resourceData
                                          };
...
*/
 ...
 getVehicleName(value) {
    ...
    render() {
       ...
       console.log("vehicle info", this.props.vehicles)
        console.log("vehicle selected", this.props.selectedVehicleCodeArr)
       const filteredVehicle = [];
       this.props.vehicles &&  this.props.vehicles.length > 0 && this.props.vehicles.map((veh) => {

             if(this.props.selectedVehicleCodeArr && this.props.selectedVehicleCodeArr.length > 0) {
             if(this.props.selectedVehicleCodeArr.includes(veh.name)) {
 ...
 // console.log("TScheduler vehicle - panel",this.props.vehicles);
  // console.log("TScheduler selected  vehicle",this.props.selectedVehicleCodeArr);
   // console.log("TScheduler filtered  vehicle",filteredVehicsle);
          ...
          <div className='control-wrapper drag-sample-wrapper'>
            <div className="schedule-container">
              ...
              eventSettings={{ dataSource: TripsDocData,template : this.eventTemplate.bind(this), enableMaxHeight : true,
                 ...
                 <ResourcesDirective>            <ResourceDirective field='techId' title='techId' name='Consultants' allowMultiple={false} dataSource={vehicles} textField='name' idField='techId' groupIDField="GroupId" colorField={this.bgcolor('doctype')}>
                                                    </ResourceDirective>
               </ResourcesDirective>
                <ViewsDirective>

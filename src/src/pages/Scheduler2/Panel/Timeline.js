...
function Timeline(props) {
    ...
    const optimizeRoute = (data) => {
        ...
        if (autooptimise) {
            ...
            jsonUrl = `/json?computeBestOrder=true&routeRepresentation=summaryOnly&computeTravelTimeFor=all&routeType=shortest&avoid=unpavedRoads&travelMode=truck&vehicleMaxSpeed=0&vehicleWeight=0&vehicleLength=0&vehicleWidth=0&vehicleHeight=0&vehicleCommercial=true&vehicleLoadType=otherHazmatGeneral&vehicleEngineType=combustion&key=ctZzLlfGUpaNdfHiIobOeub8NBzzGkNG`;
                ...
                if (summaryResult && summaryResult.summarydata && summaryResult.serviceTime && summaryResult.waitingTime) {
                    ...
                    if (results) {
                        ...
                        if (props.data && legs && props.data.stops < results[0].legs.length) {
                            ...
                            let maxTotTimeSec = convertHrToSec(Number(props.data.techObject.maxtotaltime));
                            if (prevTripsDist >= props.data.techObject.maxtotaldist
                                || prevTripsTime >= maxTotTimeSec
                                || optimizationStatus
                                || tripsClosed) {
                                if (prevTripsDist >= props.data.techObject.maxtotaldist) {
                                    setDistErrorMessage(`The vehicle cannot perform trip more than ${props.data.techObject.maxtotaldist} KM, please review trip documents.`)
                                    setDistError(true);
                                } else if (prevTripsTime >= maxTotTimeSec) {
                                    setTimeErrorMessage(`The vehicle cannot perform trip more than ${props.data.techObject.maxtotaltime} Hrs, please review trip documents.`)
                                    setTimeError(true);
                                } else if (tripsClosed) {
                                    setTripsClosedErrorMessage(`The Vehicle ${props.data.techObject.codeyve} cant perform this Trip on ${moment(props.date).format('yyyy-MM-DD')} , Prev Trips covered whole Day. Use Another Vehicle or Schedule on Next Day`)
    ...
    const displayTomTomDetails = (data) => {
        ...
        if (lang === "en") {
            ...
            details = {
                ...
                "Max Total Distance": data.techObject.maxtotaldist + ' ' + distunts,


                "</br><b>Time</b></br>Trip Time": tottime,
                "Max Total Time": data.techObject.maxtotaltime + ' Hours',


                "</br><b>Capacity</b></br>Total Weight": totweight,
                "Max Vehicle Capacities": data.techObject.capacities + ' ' + massunit,

                "</br><b>Volume</b></br>Total Volume": totvolume,
                "Max Volume": data.techObject.vol + ' ' + volunits,

                "</br><b>Parameters</b></br>Max Speed": data.techObject.maxspeed + ' ' + distunts + '/Hr',
                "Vehicle Length": (Number(data.techObject.length)) / 100 + ' Mts',
                "Vehicle Width": (Number(data.techObject.width)) / 100 + ' Mts',
                "Vehicle Height": (Number(data.techObject.heigth)) / 100 + ' Mts'
            }
        }
        else {
            ...
            details = {
                ...
                "Distance Maxi": data.techObject.maxtotaldist + ' ' + distunts,


                "</br><b>Time</b></br>Duree de la tournee": tottime,
                "Duree Totale": data.techObject.maxtotaltime + ' Hours',


                "</br><b>Capacity</b></br>Poids chargement": totweight,
                "poids Maxi": data.techObject.capacities + ' ' + massunit,

                "</br><b>Volume</b></br>Total Volume": totvolume,
                "Max Volume": data.techObject.vol + ' ' + volunits,

                "</br><b>Parameters</b></br>vitesse Maxi": data.techObject.maxspeed + ' ' + distunts + '/Hr',
                "Longeur": (Number(data.techObject.length)) / 100 + ' Mts',
                "Largeur": (Number(data.techObject.width)) / 100 + ' Mts',
                "Hauteur": (Number(data.techObject.heigth)) / 100 + ' Mts'
                    ...
                    <div className="table-responsive">
                        <Table className="mb-0">
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">{props.t('Technician')}</th>

...
class WCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      fetch(
        "http://tmsx3.tema-systems.com:8050/api/fms/report/usrsites?user=" +
          user.username
      ),
      fetch(
        "http://tmsx3.tema-systems.com:8050/api/fms/report/tripslist?date=" +
          date
      ),
    ])
      .then(([sites, mapTrips]) => {
  ...
  onMapDateAndSiteChange = (seldate, sites) => {
    Promise.all([
      fetch(
        `http://tmsx3.tema-systems.com:8050/api/fms/report/tripslistbyDateandSite?date=${seldate}&site=${sites}`
      ),
    ])
      .then(([trips]) => {
  ...
  onMapDateChange = (seldate, camefrom) => {
    ...
    if (this.state.selectedSitesArr.length > 0 && camefrom !== "uncheck") {
      ...
      Promise.all([
        fetch(
          `http://tmsx3.tema-systems.com:8050/api/fms/report/tripslist?date=${seldate}`
        ),
      ])
        .then(([trips]) => {
          return Promise.all([trips.json()]);
  ...
  OnSiteSelection = (selSite, from) => {
    ...
    Promise.all([
      fetch(
        `http://tmsx3.tema-systems.com:8050/api/fms/report/vehicle?site=${selSite}`
      ),
      fetch(
        `http://tmsx3.tema-systems.com:8050/api/fms/report/route?site=${selSite}&start=${startDate}&end=${endDate}`
      ),
    ])
      .then(([vehicles, data]) => {
  ...
  RefreshDataforWeek = () => {
    ...
    if (this.state.selectedSite) {
      Promise.all([
        fetch(
          `http://tmsx3.tema-systems.com:8050/api/fms/report/route?site=${this.state.selectedSitesArr}&start=${startDate}&end=${endDate}`
        ),
      ])
        .then(([data]) => {

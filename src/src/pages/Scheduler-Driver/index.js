...
import TripsCard from './Panel/TripsCard';
 ...
 UnlockConfirmTrip = (ClickedTrip) => {
  ...
  OptimiseConfirmTrip = (ClickedTrip) => {
    ...
    submitTrips = (trips) => {
      ...
      fetch('http://localhost:3001/api/v1/transport/trips', {
        method: 'POST',
...
refreshAllPanels = () => {
 ...
 const currDate = moment.tz(this.state.date,'').format('YYYY-MM-DD');
       ...
       fetchAPI(this.state.selectedMultipleSites, currDate)
                    .then(([res1, res2, res3]) => {
                      this.setState({
                        vehiclePanel: res1,
                        docsPanel : res2,
...
updateTripsPanel = (currMarkers, currGeoData, i) => {
  ...
  handleDragStart = (event, valueObj, type, index, id) => {
    ...
    if (type === "vehicle") {
      ...
      const currDate = moment.tz(this.state.date,'').format('YYYY-MM-DD');
      const url = 'http://localhost:3001/api/v1/transport/prevtrpsite?veh=' + valueObj.codeyve + '&date=' + currDate;
      fetch(url)
        .then(function (response) {
...
onValidateAll = ()=>{
   this.setState({loader : true});
    ...
    fetch('http://localhost:3001/api/v1/transport/groupvalidate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
 ...
 Nonvalidate = (i) => {
  this.setState({loader : true});
     // console.log("s1 - inside non validate");
         ...
         fetch('http://localhost:3001/api/v1/transport/nonvalidate', {
           method: 'POST',
           headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify(trips)
         }).then((response) => {
   ...
   validate = (i) => {
     this.setState({loader : true});
       ...
       var ClickedTrip = tripsPanels[i];
       let trips = ClickedTrip;
       fetch('http://localhost:3001/api/v1/transport/validate', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify(trips)
       }).then((response) => {
...
console.log("Tktk1 inside moment - date",moment(date).format('YYYY-MM-DD'));
  ...
  onDocmessage = (docNum,msg, Msgtype) =>{
   ...
   // var deleteddoc = this.state.;
    fetch('http://localhost:3001/api/v1/transport/update/deldoc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
 ...
 var currDate = moment.tz(this.state.date,'');
  ...
  lockTrip = (trips, index) => {
    ...
    fetch('http://localhost:3001/api/v1/transport/lock/trips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(trips)
    }).then((response) => {
...
*/
   ...
   onWarningAlertOff = (index , tripcode) => {
       ...
       var trip = tripsPanel[index];
           trips.push(trip);
       const currDate = moment.tz(this.state.date,'').format('YYYY-MM-DD');
       fetch('http://localhost:3001/api/v1/transport/alertoff/trip', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify(trips)
       }).then((response) => {
 ...
 var dropObject = [], pickupObject = [],drops = 0, pickups = 0;
  ...
  grouplockTrips = () => {
   ...
   var tripsPanel = this.state.tripsPanel;
      ...
      fetch('http://localhost:3001/api/v1/transport/lock/multipletrips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
 ...
 autoResetTrips=  () => {
     ...
     // console.log("Trips are reeadyy to delete =",unlockedTrips);
        fetch('http://localhost:3001/api/v1/transport/delete/trips', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(unlockedTrips)
        }).then((response) => {
...
updatePOPreReceiptDocs = (trip) => {
  var trips = trip;
 // trips.push(trip);
  fetch('http://localhost:3001/api/v1/transport/freq/trips', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(trips)
      }).then((response) => {
  ...
  deleteTrip = (trips, index) => {
    ...
    const currDate = moment.tz(this.state.date,'').format('YYYY-MM-DD');
    let value = this.state.selectedMultipleSites
    fetch('http://localhost:3001/api/v1/transport/delete/trip', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(trips)
    }).then((response) => {

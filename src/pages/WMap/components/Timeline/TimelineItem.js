...
const TimelineItem = ({ data }) => {
  ...
  if (data.type === 'PRECEIPT') {
    url = { x3Url } + "/$sessions?f=GESXX10CPTH/2//M/" + data.custid
  }
  else if (data.type === "DEP-SITE" || data.type === "RETURN-SITE") {
    url = { x3Url } + "/$sessions?f=GESFCY/2//M/" + data.custid
  }
  else if (data.type === "DLV") {
    url = { x3Url } + "/$sessions?f=GESSDH/2//M/" + data.custid
  }
  else if (data.type === 'PICK') {
    url = { x3Url } + "/$sessions?f=GESPRH2/2//M/" + data.custid
  }
  else {
  }

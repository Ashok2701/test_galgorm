...
const WeekCalender = (props) => {
  ...
  const updateWeekDates = (selectedDate) => {
    ...
    const startOfWeek = moment(currentDate).local().startOf('isoWeek');

    // Get the end of the week containing the selected date
    const endOfWeek = moment(currentDate).local().endOf('isoWeek');
  ...
  return (
    <div>
      <Tooltip title="Previous">
        <IconButton>
          <ChevronLeftIcon
          style={{backgroundColor:"#007b788f"}}
            className=" rounded-circle text-white font-bold"
      ...
      <Tooltip title="Next">
        <IconButton>
          <ChevronRightIcon
          style={{backgroundColor:"#007b788f"}}
            className="font-bold text-white rounded-circle"

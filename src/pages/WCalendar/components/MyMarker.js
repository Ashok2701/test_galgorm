...
export default function MyMarker(props) {
 ...
 const getlink = (type,id) => {
  if(type === 'PRECEIPT') {
   url = "https://galgormx3.cpio.cloud/syracuse-main/html/main.html?url=/trans/x3/erp/TEST/$sessions?f=GESXX10CPTH/2//M/" + id
    return (
        <a target="_blank" href={url}>{id}</a>
    )
  }
  else if(type === "DEP-SITE" || type === "RETURN-SITE"){
   url = "https://galgormx3.cpio.cloud/syracuse-main/html/main.html?url=/trans/x3/erp/TEST/$sessions?f=GESFCY/2//M/" + id
   return (
          <a target="_blank" href={url}>{id}</a>
      )
  }
  else if(type === "DLV"){
   url = "https://galgormx3.cpio.cloud/syracuse-main/html/main.html?url=/trans/x3/erp/TEST/$sessions?f=GESSDH/2//M/" + id
   return (
          <a target="_blank" href={url}>{id}</a>
      )
  }
  else if(type === 'PICK'){
   url = "https://galgormx3.cpio.cloud/syracuse-main/html/main.html?url=/trans/x3/erp/TEST/$sessions?f=GESPRH2/2//M/" + id
   return (
          <a target="_blank" href={url}>{id}</a>
      )

...
class EditorTemp extends React.PureComponent {
  ...
  getDocink = (tripData) => {
    ...
    if (tripData.doctype == 'PRECEIPT') {
      url = `${x3Url}/$sessions?f=GESXX10CPTH/2//M/` + tripData.docnum;
      ...
      url = `${x3Url}/$sessions?f=GESSDH/2//M/` + tripData.docnum;
      return (<a name="Subject" className="e-field e-input" target="_blank" href={url}><span style={{ color: 'black', fontSize: '14px' }}>{tripData.docnum}</span></a>
      );
    }
    else if (tripData.doctype == 'PICK') {
      url = `${x3Url}/$sessions?f=GESPRH2/2//M/` + tripData.docnum;
      return (<a name="Subject" className="e-field e-input" target="_blank" href={url}><span style={{ color: 'black', fontSize: '14px' }}>{tripData.docnum}</span></a>
      );
    }
    else if (tripData.doctype == 'MISCPICK') {
      url = `${x3Url}/$sessions?f=GESXX10CMIS/2//M/` + tripData.docnum;
      return (<a name="Subject" className="e-field e-input" target="_blank" href={url}><span style={{ color: 'black', fontSize: '14px' }}>{tripData.docnum}</span></a>
      );
    }
    else {
      url = `${x3Url}/$sessions?f=GESFCY/2//M/` + tripData.docnum;
      return '';
    }
